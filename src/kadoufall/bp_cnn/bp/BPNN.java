...
public class BPNN {
     ...
     * @param outputNum
     */
    public BPNN(int inputNum, int hiddenNum, int outputNum) {
    ...
    public double train(double[] inputValue, double[] destination) {
        ...
        for (int i = 0; i < hiddenNum; i++) {
            for (int j = 0; j < outputNum; j++) {
                double delta = rate * (destination[j] - outputValue[j]) * outputValue[j] * (1 - outputValue[j]) * hiddenValue[i];
        ...
        for (int i = 0; i < inputNum; i++) {
            for (int j = 0; j < hiddenNum; j++) {
                ...
                for (int k = 0; k < outputNum; k++) {
                    temp += hiddenOutputWeight[j][k] * outputValue[k] * (1 - outputValue[k]) * (destination[k] - outputValue[k]);
                }
                double delta = rate * hiddenValue[j] * (1 - hiddenValue[j]) * inputValue[i] * temp;
                inputHiddenWeight[i][j] += delta;
            }
        }

        for (int i = 0; i < outputNum; i++) {
            double delta = rate * (destination[i] - outputValue[i]) * outputValue[i] * (1 - outputValue[i]);
            outputBias[i] += delta;
        }
        ...
        for (int i = 0; i < hiddenNum; i++) {
            double temp = 0;
            for (int k = 0; k < outputNum; k++) {
                temp += hiddenOutputWeight[i][k] * outputValue[k] * (1 - outputValue[k]) * (destination[k] - outputValue[k]);
            }
            double delta = rate * hiddenValue[i] * (1 - hiddenValue[i]) * temp;
            hiddenBias[i] += delta;
        }

        /*
        outputValue = predict(inputValue);
        totalError = 0;
